	...
	app := s.App.Party("/").AllowMethods(iris.MethodOptions)
	{
		...
		v1 := app.Party("api/v1")
		{
			...
			v1.PartyFunc("/admin", func(admin iris.Party) { //casbin for gorm  // <- IMPORTANT, register the middleware.
				...
				admin.PartyFunc("/accounts", func(users iris.Party) {
					users.Get("/", controllers.GetAccounts).Name = "用户列表"
					users.Get("/{id:uint}", controllers.GetAccount).Name = "用户详情"
					users.Post("/", controllers.CreateAccount).Name = "创建用户"
					users.Post("/{id:uint}", controllers.UpdateAccount).Name = "编辑用户"
					users.Delete("/{id:uint}", controllers.DeleteAccount).Name = "删除用户"
				})
		...
		adminapi := app.Party("/adminapi")
		{
			// 是否开启接口请求频率限制
			if !libs.Config.Limit.Disable {
				limitV1 := rate.Limit(libs.Config.Limit.Limit, libs.Config.Limit.Burst, rate.PurgeEvery(time.Minute, 5*time.Minute))
				adminapi.Use(limitV1)
			}
			//adminapi.Use(middleware.JwtHandler().Serve, middleware.New().ServeHTTP) //登录验证
			adminapi.Get("/test", controllers.Debug)

			adminapi.PartyFunc("/client", api.ClientRouter)     // 客户
			adminapi.PartyFunc("/product", api.ProductRouter)   //商品
			adminapi.PartyFunc("/merchant", api.MerchantRouter) //
			adminapi.PartyFunc("/admin", api.AdminRouter)       //账号及管理员
		}
